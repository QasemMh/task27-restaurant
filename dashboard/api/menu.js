$(function () {
  ...
  function getAll() {
    ...
    fetch("http://localhost:81/task27_1/api/menu?limit=1000")
  ...
  $(document).on("click", ".view_item", function () {
    ...
    let uri = "http://localhost:81/task27_1/api/menu/details/" + id;
    fetch(uri)
      .then((response) => response.json())
      .then((data) => {
        /*html*/
        let view_item = `
            ...
            <img src="http://localhost:81/task27_1/image/${data.path}" class="img-fluid" alt="menu image">  
            </div>
  ...
  $(document).on("click", "#create_item", function () {
    ...
    let create_form = `
                ...
                <input  type='number' min="0" name='price' class='form-control' required/>
                 ...
                 <option value="4">Main Dishes</option>
                 </select>
                <!--item-->
  ...
  $(document).on("submit", "#create_form", function (e) {
    ...
    fetch("http://localhost:81/task27_1/api/menu/create", {
      method: "POST",
      body: formData,
    }).then((res) => {
      if (res.ok) {
  ...
  $(document).on("click", ".update_item", function () {
    let id = $(this).attr("data-id");
    let menuUri = "http://localhost:81/task27_1/api/menu/details/" + id;
  ...
  $(document).on("submit", "#update_form", function (e) {
    ...
    // Send data to backend
    fetch(
      "http://localhost:81/task27_1/api/menu/update/" + formData.get("id"),
      {
        method: "POST",
        body: formData,
      }
  ...
  $(document).on("click", ".delete_item", function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch("http://localhost:81/task27_1/api/menu/delete/" + id, {
          method: "POST",
        })
          .then((respon) => {

$(function () {
  ...
  function getAll() {
    ...
    fetch("http://localhost:81/task27_1/api/slider?limit=1000")
  ...
  $(document).on("click", ".view_item", function () {
    ...
    let uri = "http://localhost:81/task27_1/api/slider/details/" + id;
    fetch(uri)
      .then((response) => response.json())
      .then((data) => {
        /*html*/
        ...
        <img src="http://localhost:81/task27_1/image/${data.path}" class="img-fluid" alt="slider image">  
        </div>
  ...
  $(document).on("submit", "#create_form", function (e) {
    ...
    fetch("http://localhost:81/task27_1/api/slider/create", {
     method:"POST",
      body: formData,
    }).then((res) => {
      if (res.ok) {
  ...
  $(document).on("click", ".update_item", function () {
    let id = $(this).attr("data-id");
    let sliderUri = "http://localhost:81/task27_1/api/slider/details/" + id;

    fetch(sliderUri)
      .then((res) => res.json())
      .then((data) => {
        //generate create form for slider:
        /*html*/
        let update_form = `
  ...
  $(document).on("submit", "#update_form", function (e) {
    ...
    // Send data to backend
    fetch(
      "http://localhost:81/task27_1/api/slider/update/" + formData.get("id"),
      {
       method:"POST",
        body: formData,
      }
  ...
  $(document).on("click", ".delete_item", function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch("http://localhost:81/task27_1/api/slider/delete/" + id, {
         method:"POST",
        })
          .then((respon) => {

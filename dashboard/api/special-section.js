$(function () {
    ...
    function getAll() {
      ...
      fetch("http://localhost:81/task27_1/api/special?limit=1000")
    ...
    $(document).on("click", ".view_item", function () {
      ...
      let uri = "http://localhost:81/task27_1/api/special/details/" + id;
      fetch(uri)
        .then((response) => response.json())
        .then((data) => {
          /*html*/
          ...
          <img src="http://localhost:81/task27_1/image/${data.path}" class="img-fluid" alt="special image">  
          </div>
    ...
    $(document).on("submit", "#create_form", function (e) {
      ...
      fetch("http://localhost:81/task27_1/api/special/create", {
       method:"POST",
        body: formData,
      }).then((res) => {
        if (res.ok) {
    ...
    $(document).on("click", ".update_item", function () {
      let id = $(this).attr("data-id");
      let specialUri = "http://localhost:81/task27_1/api/special/details/" + id;
    ...
    $(document).on("submit", "#update_form", function (e) {
      ...
      // Send data to backend
      fetch(
        "http://localhost:81/task27_1/api/special/update/" + formData.get("id"),
        {
         method:"POST",
          body: formData,
        }
    ...
    $(document).on("click", ".delete_item", function () {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          fetch("http://localhost:81/task27_1/api/special/delete/" + id, {
           method:"POST",
          })
            .then((respon) => {
